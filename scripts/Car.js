...
//car rotation
var TINY_LEFT : int = -0.04;
var MID_LEFT : int = -0.4;
var BIG_LEFT : int = -0.9;

var TINY_RIGHT : int = 0.04;
var MID_RIGHT : int = 0.4;
var BIG_RIGHT : int = 0.9;
...
private var steerFactor : float = 0.5;

private var raceBegin : boolean = false;

private var throttleForce : float = 0.0;
private var brakeForce : float = 0.0;
...
function OnGUI()
{
	//GUI.Label(new Rect(10,70,200,20), "Steer:" + steerFactor.ToString());
}
...
//			isRaceStarted = true;		
//			throttle = 120;
//			steer = steerFactor * pxsLeapInput.GetHandAxis("Rotation");
//		}
//		else
//		{
//			if(isRaceStarted)
...
//	}
	if(raceBegin)
	{
		...
		throttle = Input.GetAxis("Vertical") * 3.0f;
		steer = Input.GetAxis("Horizontal");
	}
	else
	{
		//this.rigidbody.mass = MIN_MASS;
		throttleForce = 0.0;
		throttle = 0;
		steer = 0;
	}
	...
	CheckHandbrake();
	//Debug.Log(pxsLeapInput.GetFingerCount());
	//Debug.Log(steer);
}
...
function ApplyThrottle(canDrive : boolean, relativeVelocity : Vector3)
{
	if(canDrive && raceBegin)
	{
		...
		{
			if (!handbrake)
				//throttleForce = Mathf.Sign(throttle) * currentEnginePower * 20;
				throttleForce = 1200;
		}
		else
			//brakeForce = Mathf.Sign(throttle) * engineForceValues[0] * 20;
			 throttleForce = 1200;
		
		rigidbody.AddForce(transform.forward * Time.deltaTime * (throttleForce * Mathf.Sign(throttle)));
	}
}
...
//public interface
function setThrottle(throttleValue : boolean)
{
    raceBegin = throttleValue;
}

function getRaceBegin() : boolean
{
	return raceBegin;
}