...
public class BigBuildingBehaviour : MonoBehaviour {
    ...
    private float resizeFactor = 0.001f;
	...
	void Update () 
    {
        Debug.Log(originalCameraTransform.rotation.ToString()+originalCameraTransform.position.ToString());
        if (isAbleToRotate)
        {
            this.transform.Rotate(Vector3.up * Time.deltaTime * autoSpeed);
        }
        if (Input.GetAxis("Mouse ScrollWheel") < 0 && this.transform.localScale.x >= 0.014f)
        {
